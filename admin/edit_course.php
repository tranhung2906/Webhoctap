...
if (isset($_GET['id'])) {
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT course.*, topic.id as topic_id, topic.name as topic_name, lecturer.id as lecturer_id
        , lecturer.name as lecturer_name FROM course JOIN topic
        ON course.topic_id = topic.id JOIN lecturer 
        on course.teacher_id = lecturer.id WHERE course.id = '$id';";
        $query = mysqli_query($conn, $sql);
        $row = mysqli_fetch_assoc($query);
        //Show chủ đề
        $topic_id = $row['topic_id'];
        $truyvan = "SELECT id, name FROM topic WHERE id <> $topic_id";
        ...
        $truyvan1 = "SELECT id, name FROM lecturer WHERE id <> $lecturer_id";
        $thucthi1 = mysqli_query($conn, $truyvan1);
        while ($ketqua1 = mysqli_fetch_array($thucthi1)) {
            $arrS1[] = $ketqua1;
        }
    }
}
                ...
                <div class="container-fluid">
                    <div class="row">
                                ...
                                <form action="edit_course.php" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="topic" class="form-control" id="exampleSelect">
                                        ...
                                        <div class="form-group">
                                            <label for="exampleSelect">Giảng viên</label>
                                            <select name="giangvien" class="form-control" id="exampleSelect">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="duration" class="form-control" id="exampleInputEmail1" value="<?php echo $row['duration']  ?>">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleSelect">Trạng thái</label>
                                            <select name="status" class="form-control" id="exampleSelect">
                                                <?php if ($row['status'] == 1) { ?>
                                                    <option selected value="1">Có phí</option>
                                                    <option value="0">Miễn phí</option>
                                                <?php } else { ?>
                                                    <option selected value="0">Miễn phí</option>
                        ...
                        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                            ...
                            $chude = isset($_POST['topic']) ? mysqli_real_escape_string($conn, $_POST['topic']) : '';
                            $giangvien = isset($_POST['giangvien']) ? mysqli_real_escape_string($conn, $_POST['giangvien']) : '';
                            $mota = isset($_POST['mota']) ? mysqli_real_escape_string($conn, $_POST['mota']) : '';
                            $hocphi = isset($_POST['price']) ? mysqli_real_escape_string($conn, $_POST['price']) : '';
                            $thoigian = isset($_POST['duration']) ? mysqli_real_escape_string($conn, $_POST['duration']) : '';
                            ...
                            if ($id > 0) {
                                ...
                                $sql = "UPDATE course 
                                        SET name = '$name', teacher_id = '$giangvien', topic_id = '$chude', description = '$mota', duration = '$thoigian', price = '$hocphi', status = $status $updateImage 
                                        WHERE id = $id";
